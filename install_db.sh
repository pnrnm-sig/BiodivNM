...
then
	...
	### AJOUT gn_meta et utilisateurs
	sudo -n -u postgres -s psql -d $db_name -c "CREATE SCHEMA gn_meta AUTHORIZATION "$owner_atlas";"  &>> log/install_db.log
	sudo -n -u postgres -s psql -d $db_name -c "CREATE SCHEMA utilisateurs AUTHORIZATION "$owner_atlas";"  &>> log/install_db.log
    ...
    then
        ...
        # Import du shape des limites du territoire pour la sélection des données ($limit_shp) dans la BDD / atlas.t_layer_territoire
        ...
        # Import du shape des limites du territoire pour l'affichage ($limit_shp_limites) dans la BDD / atlas.t_layer_territoire_limites
        ogr2ogr -f "ESRI Shapefile" -t_srs EPSG:3857 data/ref/emprise_territoire_3857.shp $limit_shp_limites
        sudo -n -u postgres -s shp2pgsql -W "LATIN1" -s 3857 -D -I ./data/ref/emprise_territoire_3857.shp atlas.t_layer_territoire_limites | sudo -n -u postgres -s psql -d $db_name  &>> log/install_db.log
        rm data/ref/emprise_territoire_3857.*
        sudo -n -u postgres -s psql -d $db_name -c "ALTER TABLE atlas.t_layer_territoire_limites OWNER TO "$owner_atlas";"
        # Creation de l'index GIST sur la couche territoire atlas.t_layer_territoire_limites
        sudo -n -u postgres -s psql -d $db_name -c "ALTER TABLE atlas.t_layer_territoire_limites RENAME COLUMN geom TO the_geom; CREATE INDEX index_gist_t_layer_territoire_limites ON atlas.t_layer_territoire_limites USING gist(the_geom); "  &>> log/install_db.log

        # Import du shape des limites du territoire pour l'affichage des VP ($limit_shp_ajout) dans la BDD / atlas.t_layer_territoire_vp
        ogr2ogr -f "ESRI Shapefile" -t_srs EPSG:3857 data/ref/emprise_territoire_3857.shp $limit_shp_ajoutvp
        sudo -n -u postgres -s shp2pgsql -W "LATIN1" -s 3857 -D -I ./data/ref/emprise_territoire_3857.shp atlas.t_layer_territoire_vp | sudo -n -u postgres -s psql -d $db_name  &>> log/install_db.log
        rm data/ref/emprise_territoire_3857.*
        sudo -n -u postgres -s psql -d $db_name -c "ALTER TABLE atlas.t_layer_territoire_vp OWNER TO "$owner_atlas";"
        # Creation de l'index GIST sur la couche territoire atlas.t_layer_territoire_vp
        sudo -n -u postgres -s psql -d $db_name -c "ALTER TABLE atlas.t_layer_territoire_vp RENAME COLUMN geom TO the_geom; CREATE INDEX index_gist_t_layer_territoire_vp ON atlas.t_layer_territoire_vp USING gist(the_geom); "  &>> log/install_db.log
    ...
    ogr2ogr -f "GeoJSON" -t_srs "EPSG:4326" -s_srs "EPSG:3857" ./static/custom/territoire.json \
        PG:"host=$db_host user=$owner_atlas dbname=$db_name port=$db_port password=$owner_atlas_pass" "atlas.t_layer_territoire_limites"

    # Conversion des limites du territoire VP en json
    rm  -f ./static/custom/territoire_vp.json
    ogr2ogr -f "GeoJSON" -t_srs "EPSG:4326" -s_srs "EPSG:3857" ./static/custom/territoire_vp.json \
        PG:"host=$db_host user=$owner_atlas dbname=$db_name port=$db_port password=$owner_atlas_pass" "atlas.t_layer_territoire_vp"
    ...
    then
        ...
        ### MODIF AJOUT gn_meta et utilisateurs
        # Creation des tables filles en FWD
        echo "Création de la connexion a GeoNature pour les métadonnées"
		sudo cp data/gn2/atlas_ref_gn_meta.sql /tmp/atlas_ref_gn_meta.sql &>> log/install_db.log
        sudo sed -i "s/myuser;$/$owner_atlas;/" /tmp/atlas_ref_gn_meta.sql &>> log/install_db.log
        export PGPASSWORD=$owner_atlas_pass;psql -d $db_name -U $owner_atlas -h $db_host -p $db_port -f /tmp/atlas_ref_gn_meta.sql  &>> log/install_db.log
        # Creation des tables filles en FWD
        echo "Création de la connexion a GeoNature pour les utilisateurs"
		sudo cp data/gn2/atlas_ref_utilisateurs.sql /tmp/atlas_ref_utilisateurs.sql &>> log/install_db.log
        sudo sed -i "s/myuser;$/$owner_atlas;/" /tmp/atlas_ref_taxonomie.sql &>> log/install_db.log
        export PGPASSWORD=$owner_atlas_pass;psql -d $db_name -U $owner_atlas -h $db_host -p $db_port -f /tmp/atlas_ref_utilisateurs.sql  &>> log/install_db.log
